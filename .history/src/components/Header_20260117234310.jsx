...
const Header = () => {
  ...
  const handleScroll = (e, targetId) => {
    ...
    if (target) {
      const headerOffset = 150; // Increased from 100 to 150
  ...
  useEffect(() => {
    const handleScrollSpy = () => {
      ...
      const scrollPosition = window.scrollY + 200; // Increased from 150 to 200
  ...
  return (
    <header className="fixed top-0 left-0 w-full px-12 py-8 bg-bg-primary flex items-center justify-between z-50 max-md:px-8">
      <div className="logo-container">
        <a href="#home" className="text-text-primary text-[2.5rem] font-bold cursor-default">
          Portfolio
        </a>
      </div>
